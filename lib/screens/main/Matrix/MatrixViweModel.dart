...
import 'package:numerical_analisis/screens/result/matrix/MatricesResultScreen.dart';
...
class MatriXViewModel extends ChangeNotifier {
  ...
  void readInputs() {
    ...
    for (int i = 0; i < rowOneControllers.length; i++) {
      matrix.rowOne.add(double.parse(
          rowOneControllers[i].text.isEmpty ? "0" : rowOneControllers[i].text));
      matrix.rowTwo.add(double.parse(
          rowTwoControllers[i].text.isEmpty ? "0" : rowTwoControllers[i].text));
      matrix.rowThree.add(double.parse(rowThreeControllers[i].text.isEmpty
          ? "0"
          : rowThreeControllers[i].text));
    }
  }
  ...
  void gaussElimination(BuildContext context) {
    List<Matrix> matrices = calcGaussElimination();
    provider!.updateDataForGauss(
        x1, x2, x3, m21, m31, m32, matrices, "Gauss Elimination");
    Navigator.of(context)
        .push(MaterialPageRoute(builder: (context) => MatricesResultScreen()));
  }

  void calcMatrixWithLU(BuildContext context) {
    List<Matrix> matrices = calcGaussElimination();
  ...
  void calcGaussJordan(BuildContext context) {
    valid();
    List<Matrix> matrices = [];
    matrices.add(matrix.copyMatrix());
    ...
    provider!.updateDataForGauss(
        x1, x2, x3, m21, m31, m32, matrices, "Gauss Jordan");
    Navigator.of(context)
        .push(MaterialPageRoute(builder: (context) => MatricesResultScreen()));
  }

  void calcCramer(BuildContext context) {
    List<Matrix> matrices = [];
    matrices.add(matrix.copyMatrix());
    ...
    provider!.updateDataForCramer(x1, x2, x3, aEquivalent, a1Equivalent,
        a2Equivalent, a3Equivalent, matrices, "Cramer");
    Navigator.of(context)
        .push(MaterialPageRoute(builder: (context) => MatricesResultScreen()));
  }

  double calcAEquivalent(Matrix matrix) {
    double x = (matrix.rowOne[0] *
            ((matrix.rowTwo[1] * matrix.rowThree[2]) -
                (matrix.rowTwo[2] * matrix.rowThree[1]))) -
        (matrix.rowOne[1] *
            ((matrix.rowTwo[0] * matrix.rowThree[2]) -
                (matrix.rowTwo[2] * matrix.rowThree[0]))) +
        (matrix.rowOne[2] *
            ((matrix.rowTwo[0] * matrix.rowThree[1]) -
                (matrix.rowTwo[1] * matrix.rowThree[0])));
    return x;
  }
}
